...
public class ProceduralTextureGeneration : MonoBehaviour
{
    ...
    public void Start()
    {
        if (material == null)
        {
            ...
            if (renderer == null)
            {
                Debug.LogWarning("Cannnot find a renderer.");
                return;
            }
            ...
            for (int h = 0; h < textureWidth; h++)
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    for (int j = 0; j < 3; j++)
                    {
                        ...
                        Color color = _MixColor(circleColor, new Color(pixel.r, pixel.b, pixel.b, 0.0f), Mathf.SmoothStep(0f, 1.0f, dist * edgeBlur));
